...
export default function MyJobsTab({ staff, bookings, setBookings, isMobile, C }) {
  ...
  const shareJobCard = async (b) => {
    const lines = [
      ...
      !b.isContractVisit ? `Property: ${[b.propertyType, b.size].filter(Boolean).join(' · ') || '—'}` : '',
      `Floor / Lift: ${b.floor || '—'}`,
      `Parking: ${b.parking || '—'}`,
      `Keys: ${b.keys || '—'}`,
      b.bathrooms ? `Bathrooms: ${b.bathrooms}` : '',
      `Add-ons: ${b.addons?.length ? b.addons.map(a => a.name || a).join(', ') : 'None'}`,
      !b.isContractVisit ? `Pets: ${b.hasPets ? `Yes — ${b.petTypes || 'not specified'}` : 'No'}` : '',
      ``,
            ...
            <div ref={cardRef} style={{ padding: '24px 24px 20px', background: '#fff' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '110px 1fr', gap: '8px 12px', marginBottom: 20 }}>
                {[
                  ...
                  !jobCard.isContractVisit && ['Property', [jobCard.propertyType, jobCard.size].filter(Boolean).join(' · ') || '—'],
