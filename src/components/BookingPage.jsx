    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ paddingRight: isMobile ? 0 : 40, paddingTop: isMobile ? (step === 1 ? 0 : 16) : 80, display: isMobile ? 'block' : 'flex', flexDirection: 'column' }}>
          ...
          {step === 2 && <BookingStep1 booking={booking} onUpdate={update} onNext={() => goToStep(booking.pkg?.id === 'office_cleaning' || booking.pkg?.isHourly ? 4 : 3)} onBack={() => goToStep(1)} />}
          {step === 3 && <BookingStep1b booking={booking} onUpdate={update} onNext={() => goToStep(4)} onBack={() => goToStep(2)} />}
          {step === 4 && <BookingStep2 booking={booking} onUpdate={update} onNext={() => goToStep(5)} onBack={() => goToStep(booking.pkg?.id === 'office_cleaning' || booking.pkg?.isHourly ? 2 : 3)} isMobile={isMobile} />}
