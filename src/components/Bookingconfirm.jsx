export default function BookingConfirm({ booking, result, onClose }) {
  const { bookingRef, deposit, remaining } = result || {};
  ...
  return (
    <div style={{
      ...
      <div style={{
        background: '#FAF8F4', maxWidth: 480, width: '100%',
        padding: '40px 36px', textAlign: 'center',
      }}>
        {/* Icon */}
        <div style={{
          fontFamily: "'Cormorant Garamond',serif", fontSize: 40,
          color: '#c8b89a', marginBottom: 16,
        }}>✦</div>
        ...
        <p style={{
          fontFamily: "'Jost',sans-serif", fontSize: 13, color: '#5a4e44',
        ...
        {/* Details */}
        <div style={{
          background: '#F2EDE5', padding: '16px 20px',
          textAlign: 'left', marginBottom: 20,
        }}>
          {[
            { l: 'Service',       v: `${booking.pkg?.name}${booking.size ? ' · ' + booking.size.label : ''}` },
            { l: 'Date',          v: booking.cleanDateDisplay },
            { l: 'Time',          v: booking.cleanTime },
            { l: 'Address',       v: `${booking.addr1}, ${booking.postcode}` },
            { l: 'Deposit paid',  v: `£${deposit}`,   gold: true },
            { l: 'Balance on clean day', v: `£${remaining}` },
          ].map(row => (
            <div key={row.l} style={{
        ...
        {bookingRef && (
          <p style={{
            ...
            color: '#8b7355', letterSpacing: '0.06em', marginBottom: 24,
          }}>
            Booking reference: <strong style={{ color: '#1a1410' }}>{bookingRef}</strong>
          </p>
        )}
